<template>
  <div class="introduction">
    <div class="content-title">金币充值记录</div>
    <div class="block">
      <div class="inputSize">
    ...
    <el-table :data="tableData" border style="width:100%">
      <el-table-column prop="pay_orderId" label="订单号"></el-table-column>
      <el-table-column prop="uid" label="用户ID"></el-table-column>
      <el-table-column prop="rmbAmount" label="充值金额"></el-table-column>
      <el-table-column prop="num" label="获得金币"></el-table-column>
      <el-table-column prop="dateTime" label="充值时间" :formatter="dateFormat"></el-table-column>
    </el-table>
    <el-pagination
...
<script>
import { PlayerChargeList } from "@/api/management";
...
export default {
  ...
  methods: {
    init() {
      ...
      PlayerChargeList(_this.formData).then(resp => {
    ...
    search() {
      var _this = this;
      if (!_this.formData.orderId&&(!_this.formData.start || !_this.formData.end)) {
    ...
    initDate() {
      if (this.formData.orderId == "") {
        this.formData.start = moment()
          .subtract(7,'days')
          .format("YYYY-MM-DD");
        this.formData.end = moment().format("YYYY-MM-DD");
      }
    }
  },
  watch: {
    $route(to, from) {
      if (to.path == "/playersearch/chargesearch") {
        this.init();
      }
    }
  },
